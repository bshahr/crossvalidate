...
def make_methods_list():
    ...
    for C in [1e3, 1e4, 1e5]:
        for gamma in [0.01, 0.05, 0.1]:
            ...
            methods.append(delayed(sl.svm.SVC)(kernel='poly', degree=2, C=C, gamma=gamma))
            methods.append(delayed(sl.svm.SVC)(kernel='poly', degree=3, C=C, gamma=gamma))
    ...
    for n in [10, 50, 100, 200, 500]:
        methods.append(delayed(sl.ensemble.RandomForestClassifier)(n_estimators=n, criterion='entropy'))

    # Gradient boosting.
#    for n in [10, 50, 100, 200, 500]:
#        for a in [.0001, .001, .01, .1, 1., 10.]:
#            methods.append(delayed(sl.ensemble.GradientBoostingClassifier)(n_estimators=n, learn_rate=a))
    ...
    for n in [1, 5, 10, 15, 20]:
        methods.append(delayed(sl.neighbors.KNeighborsClassifier)(n_neighbors=n, weights='distance'))
